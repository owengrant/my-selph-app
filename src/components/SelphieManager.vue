<template>
  <v-container fluid>
    <v-row>
      ...
      <v-col cols="3">
        <v-row>
          <v-col cols="12">
            <v-card>
              <v-card-title> Teach YourSelph </v-card-title>
              <v-divider />
              <v-card-text>
                <v-form>
                  <v-text-field
                  ...
                  <audio v-if="hideResponse()" controls>
                    <source
                      :src="audioSource(currentResponse)"
                      type="audio/mpeg"
                    />
                  </audio>
                </v-form>
    ...
    <v-dialog v-model="showCreate" width="500">
      ...
      <v-card>
        <v-card-title> Create Selphie </v-card-title>
        <v-card-text>
          <v-form>
            <v-text-field v-model="question" label="Enter the question" />
            <v-file-input
              v-model="response"
              prepend-icon=""
          ...
          <v-card-actions class="pa-0">
            <v-btn @click="createSelphie" color="primary"> Create </v-btn>
          </v-card-actions>
        </v-card-text>
      </v-card>
    </v-dialog>
...
export default class SelphieManager extends Vue {
  ...
  hideResponse() {
    ...
    if (result == false) this.currentResponse = null;
    return result;
  }
}
</script>
